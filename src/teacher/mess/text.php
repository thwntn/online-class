<?php
    ...
    $query = "SELECT chat_id FROM chat WHERE user_name = '$user' AND friend_user = '$user_friend'";

    foreach($con -> query($query) as $value) {
        $idSend = $value['chat_id'];
    }
    
    //lấy chat id tin nhắn received
    $query = "SELECT chat_id FROM chat WHERE user_name = '$user_friend' AND friend_user = '$user'";
